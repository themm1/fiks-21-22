...
int dfs(unordered_map<string, node> &graph,
        unordered_map<string, int> &made_counts, string node, int in_edge_val,
        int total) {
    ...
    if (graph[node].produced_count == 0)
        return needed_count;
    int made_count =
        ceil((float)needed_count / (float)graph[node].produced_count);
